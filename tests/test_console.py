...
class TestHBNBCommand(unittest.TestCase):
    """A test class for the console (command interpreter).
    """
    @classmethod
    def setUpClass(cls):
        cls.storage_type = os.getenv('HBNB_TYPE_STORAGE')

    def test_create_command(self):
        if self.storage_type != 'db':
            self._test_fs_create()
        else:
            self._test_db_create()

    def _test_fs_create(self):
